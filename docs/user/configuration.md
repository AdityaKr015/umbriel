...
hide_timeout = 0  # seconds without pointer activity; 0 disables hiding
```
...
Set `hide_timeout` to a value from `1` to `3600` to hide the cursor after that
many seconds without pointer activity. Motion, clicks, scrolling, and tablet
input reveal it and restart the timeout.
