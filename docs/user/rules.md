...
| `default_maximize` | bool | Explicitly open maximized. Umbriel ignores maximized state restored by a client while it opens, but honors later client requests. Tiled windows expand their column to full width without changing the layout; floating windows fill the usable area. |
