...
namespace umbriel {
  ...
  // split; when there is no splittable leaf they are null/0 and `column` falls back to the append index. hintBox is
  // clamped to the target output's usable area; zero-sized means "draw nothing".
  struct DropTarget {
  ...
  // `scroll` is the horizontal offset the caller renders the workspace at. `worldX`/`worldY` are layout coordinates in
  // that same frame. Derives the usable area from workspace.group()->output() internally.
