...
LockSurface::LockSurface(
    Server& server, wlr_session_lock_surface_v1* lockSurface, wlr_scene_tree* parent)
...
wlr_surface* LockSurface::surface() const {
  return m_lockSurface != nullptr ? m_lockSurface->surface : nullptr;
}
...
void LockSurface::configure() {
  ...
  wlr_session_lock_surface_v1_configure(m_lockSurface, static_cast<uint32_t>(width),
...
void LockSurface::handleOutputCommit() {
  configure();
}

SessionLock::SessionLock(Server& server, wlr_session_lock_v1* lock)
    : m_server(&server), m_lock(lock) {
  m_lock->data = this;
...
void SessionLock::removeSurface(LockSurface* surface) {
  std::erase_if(m_surfaces, [surface](const std::unique_ptr<LockSurface>& entry) {
...
void SessionLock::handleNewSurface(void* data) {
  ...
  kLog.debug(
      "lock surface output={}",
      lockSurface->output != nullptr ? lockSurface->output->name : "(none)");
  m_surfaces.push_back(
      std::make_unique<LockSurface>(*m_server, lockSurface, m_server->lockTree()));
}
