#!/usr/bin/env bash
# Closing a window that is off-screen to the left does not shift the visible ones.
#
...
# Workspace::detachFromLayout before the column goes.
#
# The setup is fussy for a reason, and the fussiness is the check. Removal is
# followed by ensureFocusedVisible, which clamps the scroll into the focused
# column's visible band; that clamp absorbs the whole shift whenever the scroll
