...
readonly CONFIG_BACKUP="$UMBRIEL_CONFIG.drag-left-hint.bak"
CLIENT_PIDS=()
POINTER_PID=
...
sleep 0.6
pointer move "$start_x" "$start_y" press "$BTN_LEFT" move "$OVERFLOW_LEFT" 360 pause 1200 release "$BTN_LEFT" &
POINTER_PID=$!
sleep 0.5
