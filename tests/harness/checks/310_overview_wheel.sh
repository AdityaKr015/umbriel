...
# The wheel, fallback vertical arrows and the three-finger swipe all reach Overview::selectRelativeWorkspace. This
# check exercises the wheel path, while 346_overview_keybind_actions covers arrow input. The headless backend has no
# touchpad, and zwlr_virtual_pointer_v1 carries motion, buttons and axes but no gesture events, so the gesture state
# machine is not reachable without a real device. The active workspace is observed through ext-workspace-v1.
set -euo pipefail
